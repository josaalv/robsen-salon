/* ROBSEN · Agendamiento para clientas (vista pública premium) */
function ScreenBooking() {
  ...
  const online = RB.servicios.filter(s => s.online);
  const profs = srv ? srv.prof.map(getEst) : [];
  const anticipo = srv && srv.anticipo ? Math.round(srv.precio * 0.35) : 0;
  const slots = ['09:00', '09:30', '10:30', '11:00', '12:30', '13:00', '14:00', '16:00', '17:00', '18:00'];
  const off = ['10:30', '14:00'];

  const steps = [['Servicio', 'scissors'], ['Profesional', 'user'], ['Fecha y hora', 'calendar-blank'], ['Tus datos', 'identification-card'], ['Confirmación', 'check-circle']];
      ...
      <div className="book-aside">
        ...
        <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto' }}>
            <h2 className="display" style={{ fontSize: 30, lineHeight: 1.15 }}>Reserva tu<br /><span className="gold-text">experiencia de belleza</span></h2>
            <p className="muted" style={{ fontSize: 13.5, lineHeight: 1.6, marginTop: 14, maxWidth: 320 }}>Agenda en línea en unos pasos. Apartas tu lugar con un anticipo y nuestro equipo confirma tu cita personalmente.</p>
          </div>

          <div style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 2 }}>
            {steps.map(([t, ic], i) => {
              const n = i + 1, st = step === n ? 'active' : step > n ? 'done' : '';
              return <div key={t} className={'step-dot ' + st}><span className="n">{step > n ? <Ic n="check" /> : n}</span>{t}</div>;
            })}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 640, width: '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          {step === 1 && (<div className="mt14">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 420, overflowY: 'auto', paddingRight: 6 }}>
              {online.map(s => (
                <div key={s.id} className={'opt-card' + (srv && srv.id === s.id ? ' sel' : '')} onClick={() => { setSrv(s); setProf(null); }}>
                  <div className="f1"><div style={{ fontWeight: 600, fontSize: 14.5 }}>{s.nombre}</div><div className="vc gap12 mt6" style={{ fontSize: 12, color: 'var(--text-3)' }}><span className="badge neutral" style={{ fontSize: 10.5 }}>{s.cat}</span><span className="vc" style={{ gap: 4 }}><Ic n="clock" />{s.dur} min</span></div></div>
          ...
          {step === 2 && (<div className="mt14">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div className={'opt-card' + (prof === 'any' ? ' sel' : '')} onClick={() => setProf('any')}>
                ...
                <div className="f1"><div style={{ fontWeight: 600, fontSize: 14.5 }}>Sin preferencia</div><div className="dim" style={{ fontSize: 12, marginTop: 3 }}>Asignamos a la mejor estilista disponible</div></div>
                <div className="check"><Ic n="check" /></div>
              </div>
              {profs.map(p => (
                <div key={p.id} className={'opt-card' + (prof === p.id ? ' sel' : '')} onClick={() => setProf(p.id)}>
                  <Avatar ini={p.ini} color={p.color} />
                  <div className="f1"><div style={{ fontWeight: 600, fontSize: 14.5 }}>{p.nombre}</div><div className="dim" style={{ fontSize: 12, marginTop: 3 }}>{p.rol} · 4.9 ★</div></div>
                  <div className="check"><Ic n="check" /></div>
                </div>
              ))}
            </div>
          ...
          {step === 3 && (<div className="mt14">
            ...
            <div className="vc gap8" style={{ marginBottom: 20, flexWrap: 'wrap' }}>
              {[['Mié', 11], ['Jue', 12], ['Vie', 13], ['Sáb', 14], ['Lun', 16], ['Mar', 17]].map(([d, n]) => (
                <div key={n} className={'timeslot' + (dia === n ? ' sel' : '')} style={{ minWidth: 64 }} onClick={() => setDia(n)}>
                  <div style={{ fontSize: 11, fontWeight: 600, opacity: .8 }}>{d}</div><div className="serif" style={{ fontSize: 18 }}>{n}</div>
                </div>
              ))}
            </div>
            <div className="eyebrow" style={{ marginBottom: 12 }}>Horarios disponibles · Junio {dia}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10 }}>
              {slots.map(s => { const dis = off.includes(s); return <div key={s} className={'timeslot' + (hora === s ? ' sel' : '') + (dis ? ' off' : '')} onClick={() => !dis && setHora(s)}>{s}</div>; })}
            </div>
          </div>)}

          {step === 4 && (<div className="mt14">
            ...
            {anticipo > 0 && (
              <div className="card gold-edge mt18" style={{ padding: 18 }}>
                ...
                <p className="muted" style={{ fontSize: 12.5, marginTop: 10, lineHeight: 1.55 }}>Este servicio requiere un anticipo de <b className="gold-text">{mxn(anticipo)}</b>, que se descuenta del total. El resto lo pagas en el salón.</p>
              </div>
            )}
          </div>)}
          ...
          {step === 5 && (
            <div className="center" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', paddingTop: 20 }}>
              <div style={{ width: 76, height: 76, borderRadius: '50%', background: 'var(--gold-grad)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#241c0c', fontSize: 36, boxShadow: '0 16px 50px -16px var(--gold-glow)' }}><Ic n="check" w="bold" /></div>
              <h1 className="display" style={{ fontSize: 30, margin: '22px 0 8px' }}>¡Solicitud recibida!</h1>
              <p className="muted" style={{ fontSize: 14, maxWidth: 420, lineHeight: 1.6 }}>Hemos recibido tu solicitud de cita. <b style={{ color: 'var(--gold)' }}>Tu cita será confirmada por nuestro equipo</b> vía WhatsApp en las próximas horas.</p>
            </div>
          )}
            ...
            <div className="card mt24" style={{ background: 'var(--surface)', padding: '14px 18px', marginTop: 'auto' }}>
              <div className="between">
                <div className="vc gap16" style={{ flexWrap: 'wrap' }}>
                  ...
                  {prof && <span className="vc gap8" style={{ fontSize: 13, color: 'var(--text-2)' }}><Ic n="user" />{prof === 'any' ? 'Sin preferencia' : getEst(prof).nombre.split(' ')[0]}</span>}
                  {hora && <span className="vc gap8" style={{ fontSize: 13, color: 'var(--text-2)' }}><Ic n="clock" />Jun {dia} · {hora}</span>}
                </div>
          ...
          <div className="vc gap12 mt24" style={{ justifyContent: 'space-between' }}>
            {step > 1 && step < 5 ? <button className="btn ghost" onClick={back}><Ic n="arrow-left" />Atrás</button> : <span></span>}
            {step < 4 && <button className="btn gold" disabled={!canNext} style={{ opacity: canNext ? 1 : .4, pointerEvents: canNext ? 'auto' : 'none' }} onClick={next}>Continuar<Ic n="arrow-right" /></button>}
            {step === 4 && <button className="btn gold" onClick={next}><Ic n="check" />{anticipo > 0 ? `Pagar anticipo ${mxn(anticipo)}` : 'Solicitar cita'}</button>}
            {step === 5 && <button className="btn gold w100" style={{ justifyContent: 'center' }} onClick={() => { setStep(1); setSrv(null); setProf(null); setHora(null); }}>Agendar otra cita</button>}
          </div>
        </div>
      </div>
    </div>
