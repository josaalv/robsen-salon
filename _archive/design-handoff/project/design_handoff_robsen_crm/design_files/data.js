...
window.RB = (function () {
  ...
  const estadosCl = ['VIP','Frecuente','Activa','Nueva','Inactiva'];
  const clientas = [
  ...
  // Citas del día (para dashboard y agenda diaria)
  const hoy = [
  ...
  // Ventas últimos 7 días (lun-dom)
  const ventas7 = [
    { d:'Lun', v:18400 }, { d:'Mar', v:22100 }, { d:'Mié', v:16800 },
    { d:'Jue', v:27600 }, { d:'Vie', v:34200 }, { d:'Sáb', v:41800 }, { d:'Dom', v:12400 },
  ];
  // Comparativo mensual (6 meses)
  const ventasMes = [
    { d:'Ene', v:412000 }, { d:'Feb', v:438000 }, { d:'Mar', v:401000 },
    { d:'Abr', v:472000 }, { d:'May', v:518000 }, { d:'Jun', v:486000 },
  ];

  const finanzas = {
    ingresosServicio: 386400,
    ingresosProducto: 99600,
    gastos: 168200,
    anticipos: 54800,
    comisiones: 92300,
    get utilidad(){ return this.ingresosServicio + this.ingresosProducto - this.gastos - this.comisiones; },
  };
  ...
  const plantillas = [
    ...
    { id:'p3', nombre:'Reactivación',         icon:'sparkles',      txt:'Te extrañamos en Robsen, {nombre} 💛 Han pasado {dias} días. Regresa con 20% en tu próximo servicio. ¿Agendamos?' },
  ...
  // Cada producto: costo (compra), precio (venta), stock actual, mínimo (punto de reorden), uso (retail = reventa, interno = consumo en servicio)
  const marcas = ['Olaplex', 'Kérastase', 'L\'Oréal Pro', 'Wella', 'Moroccanoil', 'OPI', 'Robsen'];
  const productos = [
  ...
  // Movimientos de inventario (entradas/salidas) — alimentan el kardex
  const movimientos = [
  ...
  // Transacciones de venta de producto (ligadas a finanzas y a la clienta)
  const transacciones = [
  ...
  // Catálogo de adicionales (cargos extra que no son servicio ni producto de inventario)
  const adicionales = [
  ...
  // Ventas completas: cada ticket mezcla servicios (con su estilista), productos y adicionales.
  // estado: pagada | parcial (con anticipo) | pendiente
  const ventas = [
  ...
  return { estilistas, servicios, clientas, estadosCl, estadosCita, hoy, servMasVendidos,
           ventas7, ventasMes, finanzas, ingresosPorCategoria, mensajes, plantillas,
           modulos, roles, usuarios, productos, marcas, transacciones, movimientos,
           adicionales, ventas };
})();
...
window.RBh = (function () {
  ...
  const HOY = new Date(2026, 5, 12); // jueves 12 jun 2026 (fecha "actual" del sistema)
  const DIA = 86400000;
  const parse = (s) => { const p = s.split(' '); return new Date(+p[2], MES[p[1]], +p[0]); };
  const fmt = (dt) => dt.getDate() + ' ' + MESN[dt.getMonth()] + ' ' + dt.getFullYear();
  return {
    estilista: (id) => window.RB.estilistas.find(e => e.id === id) || {},
    mxn: (n) => '$' + Number(n).toLocaleString('es-MX'),
    mxn0: (n) => Number(n).toLocaleString('es-MX'),
    HOY,
    diasDesde: (s) => Math.round((HOY - parse(s)) / DIA),
    proxVisita: (ultima, semanas) => { const dt = parse(ultima); dt.setDate(dt.getDate() + semanas * 7); return dt; },
    fmtFecha: fmt,
    diasACumple: (cumple) => {
      const p = cumple.split(' '); let dt = new Date(HOY.getFullYear(), MES[p[1]], +p[0]);
      if (dt < HOY) dt = new Date(HOY.getFullYear() + 1, MES[p[1]], +p[0]);
      return Math.round((dt - HOY) / DIA);
    },
    // Insights de retención por clienta
    insights: (c) => {
      const dias = Math.round((HOY - parse(c.ultima)) / DIA);
      ...
      const prox = window.RBh.proxVisita(c.ultima, c.ciclo || 8);
      return { dias, cicloD, recompra, riesgo, prox, proxStr: fmt(prox), cumpleDias: window.RBh.diasACumple(c.cumple || '01 Ene') };
    },
  };
})();
