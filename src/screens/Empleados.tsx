...
const metricas: Record<string, { citas: number; ventas: number; comPct: number; rating: number; ocup: number }> = {
...
const met = (id: string) => metricas[id] || { citas: 0, ventas: 0, comPct: 30, rating: 5.0, ocup: 0 }
...
function EstilistaEditor({ est, onClose }: { est: Partial<Estilista> & { id?: string }; onClose: () => void }) {
  ...
  const [comPct, setComPct] = useState(met(est.id || '').comPct)

  const save = () => {
    const id = est.id || 'e' + Date.now()
    upsertEstilista({ id, nombre, rol, color, ini: nombre.split(' ').map((w: string) => w[0]).join('').slice(0, 2).toUpperCase() })
...
export function ScreenEmpleados({ onNavigate }: { onNavigate: (r: string) => void }) {
  ...
  const datos = estilistas.map(e => ({ id: e.id, ...met(e.id) }))
  ...
  const com = sel ? Math.round(sel.ventas * sel.comPct / 100) : 0
    ...
    <div>
      <div className="between" style={{ marginBottom: 22 }}>
        <div>
          ...
          <div className="muted" style={{ fontSize: 13, marginTop: 4 }}>{estilistas.length} profesionales activas · comisiones del mes: {mxn(comisionesMes)}</div>
        </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 360px', alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <table className="table" style={{ marginTop: 6 }}>
            ...
            <tbody>
              {datos.map(d => {
                const es = estilistas.find(x => x.id === d.id)
                if (!es) return null
                return (
                  <tr key={d.id} onClick={() => setSelId(d.id)} style={{ cursor: 'pointer', background: selId === d.id ? 'rgba(200,161,74,0.06)' : undefined }}>
                    <td>
                    ...
                    <td>
                      <div className="vc gap8">
                        <div className="bar" style={{ width: 64 }}><span style={{ width: d.ocup + '%' }}></span></div>
                        <span className="num dim" style={{ fontSize: 11.5 }}>{d.ocup}%</span>
                      </div>
                    </td>
